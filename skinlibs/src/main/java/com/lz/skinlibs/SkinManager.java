...
import com.lz.skinlibs.attr.SkinAttrSupport;
import com.lz.skinlibs.attr.SkinView;
import com.lz.skinlibs.callback.ISkinChangingCallback;
...
public class SkinManager {
    ...
    public void init(Context context, ChangeListener ChangeListener) {
        init(context);
    ...
    public void apply(Activity activity) {
        ...
        for (SkinView skinView : skinViews) {
            skinView.apply(mChangeListener);
        }
    }
    ...
    public int getThemColor() {
        int app_them = 0;
        try {
            app_them = getResourceManager().getColorRes("app_them");
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (app_them == 0) {
            app_them = getResourceManager().getColorResDefault("app_them_blue");
        }
        return app_them;
    }
